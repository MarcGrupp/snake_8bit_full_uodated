...
void push_data_2l(DATA* pdata, uint8_t data) {// passt so
    // scheis rechnerarchitektektur kann deswegen nicht 64 bit variablen verwenden
    uint8_t carry = (data & 0x03) << 6; 
    
    for (int i = 0; i < 64; i++) {
        // Sichere die untersten 2 Bits für das NÄCHSTE Byte
        uint8_t next_carry = (pdata->vek_bytes[i] & 0x03) << 6;
        
        // Schiebe nach rechts und hole den Carry von links (den hohen Bits)
        pdata->vek_bytes[i] = (pdata->vek_bytes[i] >> 2) | carry;
        
        carry = next_carry;
    }
}

int get_data_index(DATA* pdata,uint8_t index){
    uint8_t byte_index;//2,4,6,6
    byte_index = index%4;
    index = index/4;
    return ((pdata->vek_bytes[index])>>(6-(2*byte_index)))&0x03;// hier war der fehler denn byte_index 1 ist immer an stelle 87
    
}
void load_snake(char *grid,DATA *pdata, PLAYER *p){
    for(int y = 1; y <= 16; y++) {
        grid[y * 17 - 1] = '\n';
    }
...
void load_apple(char *grid,DATA *pdata, PLAYER *p){
    int posapple, freiefelder;
    if (p->bytes.p == p->bytes.a){
        freiefelder = (256-(p->player_lenght+1));
        if (freiefelder != 0){
            p->player_lenght++;
            posapple = rand()%freiefelder;
        }

        for (int i = 0;(i<16);i++){
            for (int j = 0 ;(j<16);j++){
                if(posapple == 0&&grid[i*17+j] == '.'){
                    p->posy_apple = i;
                    p->posx_apple = j;
                    goto end;
                }
                if(grid[i*17+j] != '#'&& grid[i*17+j] != '@'){
                    posapple--;
                }
    ...
    // danach updaten dass die logik mit der *** richtig ist
}

void update_pos_kopf(DATA *pdata, PLAYER *p,int direction){
...
void draw_bord(DATA *pdata, PLAYER *p,unsigned char direction){
   ...
   char grid[16*17+1];// fenster erstellen
    ...
    update_pos_kopf(pdata,p,direction);

    load_snake(grid,pdata,p);// mit überprüfung von kolesion

    load_apple(grid,pdata,p);// mit logik dass apfel nicht in schlangge spont
